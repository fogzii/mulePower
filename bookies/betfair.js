    ...
    for (const span of spans) {
      ...
      if (text.includes("Market Base Rate")) {
        ...
        return {
          commissionRate: 0.08,
          commissionDisplay:
            "Log in to Betfair to see commission (defaulting to 8%)",
        };
      }
    }
  } catch (e) {
  ...
  return new Promise((resolve) => {
    ...
    function doScrape(commission) {
      ...
      try {
        ...
        // Extract race number from the market name (e.g. "R5 1400m Hcap" → "R5")
        let raceNumber = null;
        try {
          const marketNameEl = document.querySelector("span.market-name");
          if (marketNameEl) {
            const text = marketNameEl.textContent.trim();
            const match = text.match(/\bR(\d+)\b/i);
            if (match) raceNumber = match[0].toUpperCase();
          }
        } catch (e) {
          console.warn("[Betfair Scraper] Unable to extract race number:", e);
        }
        ...
        runnerRows.forEach((row, index) => {
          try {
            ...
            if (!nameElement) {
              console.warn(
                `[Betfair Scraper] Row ${index}: No name element found`,
              );
              return;
            }
            ...
            if (!layCell) {
              console.warn(
                `[Betfair Scraper] Row ${index} (${horseName}): No lay cell found`,
              );
              return;
            }

            const layOddsElement =
              layCell.querySelector(
                'button[is-best-selection="true"] label:first-of-type',
              ) ||
            ...
            if (!layOddsElement) {
              console.warn(
                `[Betfair Scraper] Row ${index} (${horseName}): No lay odds found`,
              );
              return;
            }
            ...
            if (isNaN(layOdds)) {
              console.warn(
                `[Betfair Scraper] Row ${index} (${horseName}): Invalid lay odds "${layOddsText}"`,
              );
              return;
            }
            ...
            const layLiquidityElement =
              layCell.querySelector(
                'button[is-best-selection="true"] label:nth-of-type(2)',
              ) ||
            ...
            const liquidity = parseLiquidityText(
              layLiquidityElement
                ? layLiquidityElement.textContent.trim()
                : null,
            );
            ...
            console.error(
              `[Betfair Scraper] Error processing row ${index}:`,
              error,
            );
          }
        });
        console.log(
          `[Betfair Scraper] Successfully extracted ${horses.length} horses`,
        );
        resolve(horses);
      } catch (error) {
        ...
        setTimeout(() => {
          ...
          try {
            const closeBtn = document.querySelector(
              'button.ngdialog-close[aria-label="Dismiss"], button.ngdialog-close',
            );
            if (closeBtn) {
              closeBtn.click();
            }
